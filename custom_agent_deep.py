...
def create_env(name, *args, **kwargs):
    ...
    if env_dict:
        return NetHackMetricsEnv(
            name, 
            character=env_dict.get('character', 'valkyrie-dwarf'),
            actions_mode=env_dict.get('actions_mode', 'reduced'),
            reward_mode=env_dict.get('reward_mode', 'base'),
            test_mode=env_dict.get('test_mode',False)
        )
    else:
        return NetHackMetricsEnv(name)
...
def act(
    ...
    try:
        ...
        gym_env = create_env(flags.env, flags.env_dict, savedir=flags.rundir)
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    env = create_env(flags.env, flags.env_dict)
...
def test(flags, num_episodes=10):
    ...
    gym_env = create_env(flags.env, flags.env_dict)
    env = ResettingEnvironment(gym_env)
...
class NetHackNet(nn.Module):
    ...
    def __init__(
        ...
        """L also now equal to 8. To make it deeper. """

        """
        Change it to [32, 30, 28, 24, 16, 16, 16, 16]"""
        in_channels = [32, 16, 16, 16, 16, 16, 16, 16] # [K] + [M] * (L - 1)
        """
        Change it to [16, 16, 14, 12, 12, 10, 8, 8]
        """
        out_channels = [16, 16, 16, 16, 16, 16, 16, 8] # [M] * (L - 1) + [Y]
        ...
        """Converting it to a Deep Neural Network with 5 layers rather than two"""
...
"""
SimpleMain Method, for easy programmatic control.
"""
def SimpleMain(**kwargs):
    flags = parser.parse_args([])
    flags.env_dict = {}
    for x in ['character', 'actions_mode', 'reward_mode']:
        if x in kwargs:
            flags.env_dict[x] = kwargs[x]
    if flags.mode != 'train':
        flags.env_dict['test_mode'] = True
    for key in kwargs:
        if hasattr(flags, key):
            setattr(flags, key, kwargs[key])
    main(flags)

"""
Main method for parsing the flags, when called via command line.
